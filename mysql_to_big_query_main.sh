...
heredoc
mycount=`$mysql_connect -Bs < blob_query.txt`
...
$mysql_connect -Bs < json_query.txt | sed '$s/,$//' >> $TABLE_NAME.json
$mysql_connect $TABLE_SCHEMA -Bse "show create table $TABLE_NAME\G" > $TABLE_NAME.sql
...
# dump data
time $mysql_connect $TABLE_SCHEMA -Bse"select *  from $TABLE_NAME" > $TABLE_NAME.txt1
